...
class MarketEventsCog(commands.Cog):
    def __init__(self, bot, config):
        ...
        # Lock to prevent concurrent message updates
        self.message_update_lock = asyncio.Lock()
    ...
    async def initial_update(self):
        ...
        # Give a moment for all cogs to load and database to be ready
    ...
    async def _update_all_guilds(self):
        ...
        for guild in self.bot.guilds:
            try:
                ...
                if market_event_channel:
                    try:
                        ...
                        if market_event_channel.name != new_name:
                            await market_event_channel.edit(name=new_name)
                ...
                # Update pinned message with lock to prevent concurrent updates
                if market_times_channel:
                    async with self.message_update_lock:
                        try:
    ...
    def get_next_market_event(self, now_utc=None):
        ...
        for event in self.market_events:
            ...
            if abs((event_time - now_utc).total_seconds()) < 60:
    ...
    def format_market_times_message(self, now_utc=None):
        ...
        # Round down to last 5-minute interval for display
        ...
        for event in self.market_events:
            ...
            if event_label == "Daily Close":
                ...
                daily_close_time = now_utc.replace(hour=0, minute=0, second=0, microsecond=0)
                if now_utc.hour >= 0:  # After midnight
                    daily_close_time += timedelta(days=1)
                
                time_until = daily_close_time - now_utc
            ...
            event_time = now_utc.replace(hour=event_hour, minute=event_min, second=0, microsecond=0)
            
            # If event already passed today, calculate next occurrence
            if event_time <= now_utc:
                # Only cross out if it happened within last 3 hours
                hours_since = (now_utc - event_time).total_seconds() / 3600
                ...
                if hours_since < 3:
                    ...
                    time_until = next_event_time - now_utc
                    hours = int(time_until.total_seconds() // 3600)
                    minutes = int((time_until.total_seconds() % 3600) // 60)
        ...
        # Footer - show actual update time
        message += f"*Updated every 5 minutes* ({now_utc.strftime('%H:%M UTC')})"
