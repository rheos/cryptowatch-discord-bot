    ...
    async def admin_command(self, interaction: discord.Interaction,
        ...
        try:
            ...
            if action == "analyze":
                ...
                success = await self.bot.db.set_setting(guild.id, 'introductions_channel_id', str(interaction.channel.id))
                if success:
                    ...
                    await interaction.followup.send(
                        f"❌ Failed to set introductions channel - setting not registered in database\n"
    ...
    async def set_channels(self, interaction: discord.Interaction,
        ...
        try:
            ...
            if channel_type == "welcome":
                # Store welcome channel ID in database
                success = await self.bot.db.set_setting(guild.id, 'welcome_channel_id', str(channel.id))
                if success:
                    await interaction.followup.send(
                        f"✅ Set {channel.mention} as welcome channel\n"
                        f"New member welcome messages will be sent here"
                    )
                else:
                    await interaction.followup.send(
                        f"❌ Failed to set welcome channel - setting not registered in database\n"
                        f"Please contact the bot administrator"
                    )
                
            elif channel_type == "intro":
                # Store introduction channel ID in database
                success = await self.bot.db.set_setting(guild.id, 'introductions_channel_id', str(channel.id))
                if success:
                    await interaction.followup.send(
                        f"✅ Set {channel.mention} as introductions channel\n"
                        f"New members must post 50+ chars here to get Member role"
                    )
                else:
                    await interaction.followup.send(
                        f"❌ Failed to set introductions channel - setting not registered in database\n"
